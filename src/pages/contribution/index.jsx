...
const Contribution = () => {
  ...
  useEffect(() => {
    ...
    axios
      ...
      .then((response) => setEspece(response.data))
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (actualUser !== undefined && actualUser.role === "user") {
      const API_URL = import.meta.env.VITE_API_URL;
      ...
      let config = {
        ...
        url: `${API_URL}/contribution/create`,
        headers: {
      ...
      try {
        ...
        if (response.status === 200) {
          toast.success("Contribution effectuée avec succès");
          setTimeout(() => {
            navigate(`/espece/${id_espece}`);
          }, 3000);
        }
      } catch (error) {
